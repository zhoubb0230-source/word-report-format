...
- **自动编号（Word 多级列表 `w:numPr`）**：按「序号在不在文字里」区分——文字里没有序号（Word 生成的）
  就交给 Word 维护，不重排、不报漏编号（方案一）；文字里有**可选中的序号**（如手打「1    项目」）则视为
  手动序号，即使样式挂了自动编号也按规范改成「一、项目」。自动编号段落的缩进不一定完整写在 XML 里
  （二/三/四级常落到 Word 内置列表缩进），所以对自动编号标题/正文**强制**规范为「首行缩进2字符、左右0」，
  避免只有一级生效。
