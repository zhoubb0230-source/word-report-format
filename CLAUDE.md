...
>
> **动手改判定/应用逻辑前，查 `references/已知陷阱与设计决策.md`**——记录"看起来像 bug、其实是刻意
> 为之、别照直觉改回去"的回退陷阱（封面空格处理、updateFields 弹窗取舍、目录段落排除、pattern 标题
> 不自动改等）。**只看它顶部的「索引」表定位,再局部读相关条目(offset/limit 或 grep 区域关键词),别整篇读**;
