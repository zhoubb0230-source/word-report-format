...
9. **标题层级与图表编号判定是全文级的、且允许模型判断**：正则/样式只能给出「形状上像什么」的默认猜测
   （例如「1.」形状默认猜三级标题），但同一份文档里「1.」到底是几级标题，只有看完全文的层级关系才能确定，
   脚本本身覆盖不了所有文档的真实写法。因此这一项判定**必须**先用 `26_export_review.py` 把全文的标题/图表
   候选一次性导出（不能分片看，分片会看不到全局层级关系），模型通读后判断，需要修正的写入 `overrides.json`
   交给 `27_apply_review.py` 应用；序号连续性、字体字号等仍然是确定性计算，不受此例外影响。
...
- 产出 `<workdir>/review_candidates.json`：全文所有标题/图表候选，按文档顺序排列，每条含当前的
  （默认猜测）层级、原文本、样式名/`outlineLvl`。这个文件通常远小于全文，**直接读入模型上下文**审阅。
...
- 逐条判断：这条形状猜测的层级/图表种类对不对？错的记下 `para_index` 和正确值。
- 如果全部候选看下来都对，**不需要**再做任何事，直接进入第 3 步（`31_check_global.py` 会照既有层级算连续性）。
- 有需要修正的，写一个 `overrides.json`（**只写要改的条目，其余保持默认**）：
  ...
  `"figure"`/`"table"`/`null`（`null` = 这条其实不是图表标题）。`para_index` 必须是
  `review_candidates.json` 里已经出现过的编号——这一步只能修正已识别到的候选，不能凭空新增。
  ```bash
  python scripts/27_apply_review.py <workdir> <overrides.json路径>
  ```
  该脚本会把修正写回 `structure.json`（并重建 `shards/`），下游的字体/字号/首行缩进/序号连续性检查会
  按**修正后**的层级重新计算——层级一变，适用的字体规范也跟着变（一级黑体、二级楷体、三/四级仿宋），
  所以修正层级之后格式判定是自动刷新的，不需要额外操作。
