...
- 产出 `<workdir>/review_candidates.json`，五份按文档顺序排列的清单，这个文件通常远小于全文，
  **直接读入模型上下文**审阅：
  ...
  - `captions_confirmed` / `captions_unconfirmed`：图表标题同理——前者有题注类样式（题注/Caption/
    图表标题等）撑腰，自动改；后者仅靠「图/表+数字」文本形状匹配，只提示不改。`num_raw` 为 `null` 表示
    样式已确认是图表标题，但没找到编号数字（漏编号，而不是没识别出来）。
...
- 无法自动修的项（封面缺字段、封面绿色底、图表缺内容说明、疑似但未确认的标题/图表标题、已确认但漏编号的
  标题/图表标题）只以**批注提示**形式给出，不臆改。
