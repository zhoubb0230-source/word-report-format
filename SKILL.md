...
- `formatted.docx`：已修正 + 每处改动/提示带 `XAgent` 批注。目录刷新采用**按域打 `w:dirty` 标记**的方式
  （只给 TOC 域本身标脏，并把 `settings.xml` 的全局 `updateFields` 显式设为 `false`），用户打开时 Word 会
  **静默刷新目录、不再弹**"该文档包含的域可能引用了其他文件。是否更新…"的对话框。**不要**改回全局
  `updateFields=true`——那个全局标志正是该弹窗的来源。因目录刷新会按 TOC 样式重建条目并丢弃直接格式，
  `40_apply_fixes.py` 会同时把 `TOC1/TOC2/TOC3` 目录条目**样式本身**改成仿宋/14 磅/按级缩进（一级 0、
  二级 2 字符、三级 4 字符），保证刷新后目录仍符合规范。（若个别环境的 Word 未自动刷新，可在文档内
  Ctrl+A 全选后按 F9 手动更新目录。）
