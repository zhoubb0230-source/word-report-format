...
**进度（2026-07-25）**：
- 批次A（commit 03a68cb）：#1 #2 #3 #4 #6 #8 #9 #10 #15 + #12/#14(直接hanging部分) 已落，64→65 tests。
  4 个确认点用户已答复：#5=标题左制表位4/5/6且保页码右点线号制表位；#3=auto/480取代29.4；
  #11=四级都加、先按叠加出一版；#1=字号仍三号、严格不换算。
- #5 已落（本批次，写进 `_patch_toc_styles`=样式层，路线1"给方案C添砖"）：左制表位按级4/5/6字符
  （@21hp=840/1050/1260）+ 页码右制表位带点线号（位置=正文宽度右边界，A4=8674）。防页码换行。
- **未落**：#11（需往标题文字里插 `\t`，是内容编辑、近"不改原文"红线，且有双tab/自动编号边界，
  用户自己也说先看效果——待确认插入策略后再写）；#7（需提取层给cell标行号）；#13/#16/#17（bug，需排查，
  #17是方案C共享突变头号风险）；#12/#14继承自编号层/样式层的hanging（留给完整方案C钳编号层）。
