...
- `formatted.docx`：已修正 + 每处改动/提示带 `XAgent` 批注。**为彻底避免**打开文档时弹出"该文档包含的域
  可能引用了其他文件。是否更新…"对话框，`40_apply_fixes.py` **既不设**全局 `updateFields=true`（`settings.xml`
  里若有则显式设为 `false`），**也不给任何域打 `w:dirty` 标记**（会连模板自带的 dirty 一并清除）——因为
  **这两者都会触发该弹窗**，Word 里"打开时自动刷新目录"和"零弹窗"无法两全。代价是**目录不会在打开时自动
  刷新**：`40_apply_fixes.py` 仍会把 `TOC1/TOC2/TOC3` 目录条目**样式本身**改成仿宋/14 磅/按级缩进（一级 0、
  二级 2 字符、三级 4 字符），但要让改后的标题序号/页码体现到目录里，需用户在 Word 内 **Ctrl+A 全选后按
  F9** 手动刷新一次（`summary.md` 已写明这一步）。**排查弹窗**用 `scripts/diagnose_fields.py <docx>`，它会
  列出该文档所有弹窗来源（updateFields / dirty 域 / INCLUDE·LINK·DDE 等外部引用域 / 外部关系 / OLE）。
...
- 无法自动修的项（封面缺字段、封面绿色底、图表缺内容说明、疑似但未确认的标题/图表标题、
  章-序分组编号里漏编号的图表标题）只以**批注提示**形式给出，不臆改。**已确认样式/大纲级别的标题**若漏了
